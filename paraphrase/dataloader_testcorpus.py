...
def get_bbc_corpus_spacy(full_file_path):
    ...
    new_df = pd.DataFrame({"transcript": data_file.transcript})
    new_df["tokenized_sents"] = new_df.apply(
        lambda row: custom_spacy_tokenizer(row["transcript"]), axis=1
    )
    new_df = new_df.drop(columns=["transcript"])
    new_df = new_df.explode("tokenized_sents", ignore_index=True)
    return new_df
...
def generate_and_save_embeddings(sentences):
    ...
    with open("paraphrase/data/test_corpus1.pkl", "wb") as fOut1:
        pickle.dump(
            {"sentences": list_of_sentences, "embeddings": list_of_embeddings},
            fOut1,
...
def filter_matrixes_by_threshold_get_mean(cos_matrix, threshold):
    ...
    for value in np.arange(thr, 0.15, -0.025):
        ...
        values_above_threshold = cos_matrix[
            indices_for_similar[0], indices_for_similar[1]
        ]
    ...
    if args.save:
        ...
        if args.device == "gpu":
            if args.data == "bbc":
                ...
                sentences["sent_verbs"] = sentences["tokenized_sents"].apply(
                    lambda row: get_verbs(row)
                )
                sentences["sent_len"] = sentences["tokenized_sents"].apply(
                ...
                (
                    list_of_embeddings,
                    list_of_sentences,
                ) = generate_and_save_embeddings_bbc(sentences["tokenized_sents"])
                ...
                print(sentences.head(), sentences.shape)
                sentences["sent_verbs"] = sentences["doc"].apply(
                    lambda row: get_verbs(row)
                )
                sentences["sent_len"] = sentences["doc"].apply(
                    lambda row: get_sentence_length(row)
                )
                sentences["verb_num"] = sentences["sent_verbs"].str.len()
                sentences = sentences[sentences.sent_len > 5]
                ...
                (
                    list_of_embeddings,
                    list_of_sentences,
                ) = generate_and_save_embeddings_trump(sentences["doc"].to_list())
                print(list_of_embeddings.shape)
                ...
                (
                    ...
                    list_of_sentences,
                    list_of_labels,
                ) = generate_and_save_embeddings_custom(data_frame)
                print(list_of_embeddings.shape)
                ...
                (
                    list_of_embeddings,
                    list_of_sentences,
                ) = generate_and_save_embeddings_memsum(input_sentences)
                print(list_of_embeddings.shape)
                ...
                print(sentences[0:5])
                list_of_embeddings, list_of_sentences = generate_and_save_embeddings(
                    sentences
                )
                print(list_of_embeddings.shape)
        ...
        if args.plot:
            ...
            (
                ...
                median_values,
                num_elem,
            ) = filter_matrixes_by_threshold_get_mean(
                loaded_pair_cosine_matrix, args.threshold
            )
            plt.figure(1)
            plt.plot(thresholds, mean_values)
            plt.title(
                "Threshold vs mean cosine similarity on satisfying indices on {} data".format(
                    save_name
                ),
                fontsize=10,
                y=1.1,
            ...
            plt.title(
                "Threshold vs median cosine similarity on satisfying indices on {} data".format(
                    save_name
                ),
                fontsize=10,
                y=1.1,
            ...
            plt.title(
                "Threshold vs number of sentence pairs above threshold on {} data".format(
                    save_name
                ),
                fontsize=10,
                y=1.1,
            ...
            plt.savefig(
                "paraphrase/figs/threshold_num_{}.png".format(save_name), format="png"
            )
            ...
            (
                first_sentence_indices,
                second_sentence_indices,
            ) = filter_for_single_threshold(loaded_pair_cosine_matrix, args.threshold)
            ...
            first_sentence_indices_no_equal = np.where(
                first_sentence_indices != second_sentence_indices
            )
            second_sentence_indices_no_equal = np.where(
                second_sentence_indices != first_sentence_indices
            )
            ...
            sent_vectors1 = stored_embeddings["embeddings"][
                first_sentence_indices.tolist()
            ]
            sent_vectors2 = stored_embeddings["embeddings"][
                second_sentence_indices.tolist()
            ]
            ...
            np.save(
                "paraphrase/data/sent2_indices_{}.npy".format(save_name),
                second_sentence_indices,
            )
            np.save(
