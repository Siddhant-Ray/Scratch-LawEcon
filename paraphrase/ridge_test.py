...
def main():
    ...
    parser.add_argument(
        "-load", "--load_model", help="Specify if a model is to be loaded"
    )
    parser.add_argument(
    ...
    weighted_cosine_similarity = weighted_pairwise_cosine_sim_matrix(
        weighted_vectors, sent_vectors
    )
    print(weighted_cosine_similarity.shape)
    print(weighted_cosine_similarity[0:10])

    indices_for_sent1, indices_for_sent2 = filter_for_single_threshold(
        weighted_cosine_similarity, args.threshold
    )
