...
sent_vecs_mprc_1, sent_vecs1 = stored_data_mprc_1['embeddings'], stored_data_1['embeddings']
sent_vecs_mprc_2, sent_vecs2 = stored_data_mprc_2['embeddings'], stored_data_2['embeddings']
target_labels_mprc, target_labels = stored_labels_mprc['labels'], stored_labels['labels'],

TSNE_embedded1 = TSNE(n_components = 2, perplexity = 40, n_jobs = -1, random_state = 0).fit_transform(sent_vecs_mprc_1)
print(TSNE_embedded1.shape)

df = pd.DataFrame({'X': TSNE_embedded1[:, 0], 'Y': TSNE_embedded1[:, 1], 'Label': target_labels_mprc})
df.to_csv('paraphrase/plots/tSNE_values_mprc_vecs1.csv', index=False)

TSNE_embedded2 = TSNE(n_components = 2, perplexity = 40, n_jobs = -1, random_state = 0).fit_transform(sent_vecs_mprc_2)
print(TSNE_embedded2.shape)

df = pd.DataFrame({'X': TSNE_embedded2[:, 0], 'Y': TSNE_embedded2[:, 1], 'Label': target_labels_mprc})
df.to_csv('paraphrase/plots/tSNE_values_mprc_vecs2.csv', index=False)
