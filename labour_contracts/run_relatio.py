...
import pandas as pd
import relatio
from relatio.graphs import build_graph
from relatio.graphs import draw_graph
...
if split_sents:
    ...
    with open("labour_contracts/data/sentences.pkl", "wb") as sent_file:
        pickle.dump(
            {"id": split_sentences[0], "sentences": split_sentences[1]},
            sent_file,
...
if run_srl_method:
    ...
    with open("labour_contracts/data/srl.pkl", "wb") as sent_file:
        pickle.dump(
            {"srl": srl_res, "sentences": sentences},
            sent_file,
            protocol=pickle.HIGHEST_PROTOCOL,
        )
...
if run_narrative:
    ...
    with open("labour_contracts/data/model.pkl", "wb") as sent_file:
        pickle.dump(
            {"model": narrative_model}, sent_file, protocol=pickle.HIGHEST_PROTOCOL
        )
...
if analyse_narratives:
    ...
    df["cluster_elements"] = df.groupby(["ARG"])["ARG-RAW"].transform(
        lambda x: " | ".join(x)
    )
    ...
    df["cluster_elements"] = [
        ", ".join(set(i.split(","))) for i in list(df["cluster_elements"])
    ]
    ...
    final_statements["B-V_highdim_with_neg"] = np.where(
        final_statements["B-ARGM-NEG_highdim"] == True,
    ...
    final_statements["narrative_highdim"] = (
        final_statements["ARG0_highdim"]
        + " "
    ...
    final_statements["narrative_lowdim"] = (
        final_statements["ARG0_lowdim"]
        + " "
        + final_statements["B-V_highdim_with_neg"]
        + " "
    ...
    indexNames = final_statements[
        (final_statements["ARG0_lowdim"] == "")
        | (final_statements["ARG1_lowdim"] == "")
        | (final_statements["B-V_lowdim_with_neg"] == "")
    ].index
    ...
    sample = (
        complete_narratives[
            ~(
                (complete_narratives["narrative_highdim"].isnull())
                & (complete_narratives["narrative_lowdim"].isnull())
            )
        ]
    ...
    complete_narratives.to_csv(
        "labour_contracts/data/complete_narratives.csv", index=False
    )
