...
# Load dataset
sentiment_data_raw = pd.read_csv("data/sentiment_narratives.csv")
...
for i in range(len(sentiment_data_raw)):
    ...
    if len(annotation_text.split(" ")) == 4:
        ...
        annotation_text = (
            " ".join(word for word in list_of_words[0:2])
            + "\n"
    ...
    if len(annotation_text.split(" ")) >= 5:
        list_of_words = annotation_text.split(" ")
        annotation_text = (
            " ".join(word for word in list_of_words[0:3])
            + "\n"
    ...
    if i != 0 and i != 1:
        ...
        if i % 18 == 0:
            act_axis.annotate(
                annotation_text,
                (xcoords[i], ycoords[i]),
                (xcoords[i] + 0.03, ycoords[i] + 0.035),
        ...
        # Special case of prime index for better formattting
            ...
            act_axis.annotate(
                annotation_text,
                (xcoords[i], ycoords[i]),
                (xcoords[i] + 0.095, ycoords[i] - 0.045),
                arrowprops=dict(arrowstyle="->", connectionstyle="arc3"),
            )
        elif i % 16 == 0:
            act_axis.annotate(
            ...
            act_axis.annotate(
                annotation_text,
                (xcoords[i], ycoords[i]),
                (xcoords[i] + 0.05, ycoords[i] + 0.025),
                arrowprops=dict(arrowstyle="->", connectionstyle="arc3"),
            )
        elif i % 10 == 0:
            act_axis.annotate(
            ...
            act_axis.annotate(
                annotation_text,
                (xcoords[i], ycoords[i]),
                (xcoords[i] + 0.02, ycoords[i] + 0.005),
                arrowprops=dict(arrowstyle="->", connectionstyle="arc3"),
            )
...
for j in remaining:
    ...
    if remaining.index(j) % 2 == 0:
        act_axis.annotate(
            annotation_text,
            (xcoords[j], ycoords[j]),
            (xcoords[j] + 0.00, ycoords[j] + 0.045),
            arrowprops=dict(arrowstyle="->", connectionstyle="arc3"),
        )
    else:
        act_axis.annotate(
...
plt.show()
fig.tight_layout()
fig.savefig("figs/sentiment_narratives.pdf")
