...
def evaluate(path, clustered_frame, nclusters):
    # Load map
    mapping_df_with_labels = pd.read_csv(
        path + "mapping_data.csv",
    )
    ...
    mapping = {
        i: j
        for i, j in zip(
            mapping_df_with_labels["original"], mapping_df_with_labels["label1"]
        )
    }
    ...
    print(
        "Accuracy of numclusters = {}:".format(nclusters),
...
def run(args):
    ...
    if args.model == "kmeans":
        data_frame.to_csv(
            path
            + "manifesto_nonsimplfy_clustered_numclusters_{}.csv".format(
                args.n_clusters
            ),
            index=False,
        )
...
def main():
    ...
    parser.add_argument(
        "--n_clusters", type=int, help="Number of clusters", required=True
    )
